...
public final class IdentifierUtil {
    ...
    /** MySQL constraint identifier after safe-truncate + case rules (unquoted, for diff). */
    public static String mysqlConstraintName(String name, RulesConfig rules) {
        if (name == null || name.isBlank()) return "";
        String safe = safeConstraintName(name.trim(), 64);
        return "upper".equalsIgnoreCase(rules.getIdentifierCase())
                ? safe.toUpperCase()
                : safe.toLowerCase();
    }
