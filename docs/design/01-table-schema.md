...
禁止简陋 `CREATE TABLE`（无索引、无注释、无约束）。MySQL 脚本执行后须与 Oracle 期望 Canonical 一致（`schema verify`）。
...
- `schema export` — 双端 DDL + 快照 + coverage
- `schema import --ddl` — DDL 解析 → Canonical
- `schema diff` / `schema verify` / `schema apply`
- `schema migrate plan|apply` — 演进
